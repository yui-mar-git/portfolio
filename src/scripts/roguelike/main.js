// ===================================================
    ...
    function showScreen(target) {
      ...
      if (target) {
        ...
        if (target === mapScreen || target === document.querySelector('.battle-layout') || target === townScreen || target === shopServiceScreen || target === eventScreen || target === innScreen) {
    ...
    function showCardDetailModal(card) {
      ...
      const catPrefix = card.category && catLabelMap[card.category] ? `${catLabelMap[card.category]} ` : '';
      ...
      setTimeout(() => {
        ...
        for (let f = 0; f < generatedMap.length - 1; f++) {
          ...
          currentNodes.forEach(curr => {
            ...
            // スケール補正を適用
            const currX = ((currRect.left + currRect.width / 2) - containerRect.left + container.scrollLeft) / scale;
            ...
            nextNodes.forEach(nxt => {
              ...
              // スケール補正を適用
              const nxtX = ((nxtRect.left + nxtRect.width / 2) - containerRect.left + container.scrollLeft) / scale;
    ...
    if (btnInnRestHalf) {
      btnInnRestHalf.addEventListener('click', () => {
        if (player.gold >= 15) {
          showGameConfirm('小休憩', '15ゴールドを支払い小休憩しますか？<br>(HPとMPが最大値の50%回復します)', () => {
            player.gold -= 15;
            playSE('inn');
    ...
    if (btnInnRestFull) {
      btnInnRestFull.addEventListener('click', () => {
        if (player.gold >= 30) {
          showGameConfirm('宿泊', '30ゴールドを支払い宿泊しますか？<br>(HP・MPが全回復し、状態異常が全解除されます)', () => {
            player.gold -= 30;
            playSE('inn');
            player.hp = player.maxHp;
            player.mp = player.maxMp;
    ...
    function showFairyScreen() {
      ...
      if (document.getElementById('event-title')) document.getElementById('event-title').textContent = '妖精の泉';
      if (document.getElementById('event-text')) document.getElementById('event-text').textContent = '澄み切った泉の周りに、妖精たちが集まって遊んでいます。\n「遊んでくれるの？お礼にいいことしてあげる！」';
        ...
        const possibleEffects = [
            ...
            action: () => {
              ...
              if (unupgraded.length > 0) {
                ...
                if (index !== -1) {
                  ...
                  showGameAlert('妖精の祝福', `カード「${target.name}」が「${target.name}+」に強化された！`, () => proceedNextFloor());
                }
              } else {
          ...
          },
          {
            ...
            action: () => {
              ...
              const unowned = Object.keys(RELIC_DB).filter(r => !player.relics.includes(r) && !RELIC_DB[r].isFixed);
              if (unowned.length > 0) {
                shuffle(unowned);
                const picked = unowned[0];
                player.relics.push(picked);
                playSE('relic');
                showGameAlert('妖精の悪戯', `${dmg} ダメージを受けたが、レリック「${RELIC_DB[picked].name}」を手に入れた！`, () => proceedNextFloor());
              } else {
                showGameAlert('妖精の悪戯', `${dmg} ダメージを受けたが、見つかるレリックがなかった…`, () => proceedNextFloor());
              }
            }
          }
        ];
    ...
    function drinkMadScientistPotion() {
      ...
      if (roll < 0.25) {
        ...
        playSE('heal');
        updateHeaderBar();
        showGameAlert('大成功！', '「ヒャーッハッハ！完璧な配合だ！」\n身体の底から力が湧いてくる！最大HPが5、最大MPが3上昇した！', () => proceedNextFloor());
      } else if (roll < 0.5) {
        // 成功 (25%)
        ...
        if (subRoll < 0.5) {
          ...
          playSE('coin');
          updateHeaderBar();
          showGameAlert('成功', '「おっと、これは錬金術の副産物だ。持っていけ！」\n50ゴールドを手に入れた！', () => proceedNextFloor());
        } else {
          player.hp = player.maxHp;
          player.mp = player.maxMp;
          playSE('heal');
          updateHeaderBar();
          showGameAlert('成功', '「どうだ？疲れが吹っ飛んだだろう！」\nHPとMPが全回復した！', () => proceedNextFloor());
        }
      } else if (roll < 0.75) {
        // 失敗 (25%)
        const dmg = Math.floor(player.maxHp * 0.3) || 1;
        ...
        updateHeaderBar();
        if (player.hp <= 0) {
          showGameAlert('実験失敗…', `「あちゃー、少し刺激が強すぎたか…」\n${dmg} のダメージを受け、あなたは倒れ伏した…`);
          isGameOver = true;
          showResultOverlay(false);
        } else {
          showGameAlert('失敗', `「あちゃー、少し刺激が強すぎたか…」\n身体が焼け焦げるように熱い！ ${dmg} のダメージを受けた！`, () => proceedNextFloor());
        }
      } else {
        // 大失敗 (25%)
        const lostHp = 2;
        ...
        playSE('damage');
        updateHeaderBar();
        showGameAlert('大失敗！', `「ドッカーン！！」\n大爆発が起きた！\n最大HPが ${lostHp} 減少し、所持金が ${lostGold}G 吹き飛んだ！`, () => proceedNextFloor());
      }
    }
    ...
    const eventsList = [
      {
        ...
        image: new URL('../../assets/games/roguelike/images/characters/job_scientist_mad.png', import.meta.url).href,
    ...
    function buildCardMainText(card) {
      ...
      const fmt = (val, bVal) => (card.upgraded && val > (bVal || 0)) ? `<span class="card-dmg-val">${val}</span>` : val;
    ...
    function buildCardSubTexts(card) {
      const baseId = card.id.endsWith('+') ? card.id.slice(0, -1) : card.id;
      const base = CARD_DB[baseId] || card;
      const fmt = (val, bVal) => (card.upgraded && val > (bVal || 0)) ? `<span class="card-heal-val">${val}</span>` : val;
      const lines = [];
    ...
    function getCardCost(card) {
      let cost = card.cost;
      if ((card.category === 'spell' || card.id === 'meteor' || card.id === 'meteor+') && player.relics.includes('book_madousyo')) {
    ...
    function playCard(index) {
      ...
      if (!isSpellCard && player.actions <= 0) { logMessage('行動回数が残っていません！', 'log-system'); return; }
      ...
      if (card.type === 'attack') {
        ...
        if (card.draw) { drawCards(card.draw); player.actions++; logMessage(card.name + '！ カードを' + card.draw + '枚引く！'); }
        if (card.healSelf) { player.hp = Math.min(player.maxHp, player.hp + card.healSelf); logMessage(card.name + '！ HP +' + card.healSelf, 'log-heal'); }
        if (card.buffUp) { player.buffUp = card.buffUp; logMessage(`能昇！能昇（与ダメ+1/被ダメ-1）を${card.buffUp}ターン得た！`, 'log-heal'); }
        if (card.buffDown) { enemy.buffDown = card.buffDown; logMessage(`能降！敵に能降（与ダメ-1/被ダメ+1）を${card.buffDown}ターン付与！`, 'log-poison'); }
      }
      updateUI();
      if (enemy.hp <= 0) {
        enemy.hp = 0;
    ...
    function giveFixedRelic(relicId, cutsceneScenes, onDone) {
      ...
      showCutscene(cutsceneScenes, () => {
        ...
        showGameAlert('レリック入手', `「${relic.name}」を入手しました！<br><span class="relic-get-desc">${relic.desc}</span>`, () => {
          onDone?.();
        });
      });
    }
    ...
    function showReward() {
      ...
      const basePool = [...REWARD_POOL].filter(id => CARD_DB[id] && CARD_DB[id].category !== 'special');
