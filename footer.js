 ...
 * 共通フッターコンポーネント
 * 全ページで同じフッターを生成する。
 * SNSアイコンは href が "#" 以外の場合に "linked" クラスを付けて有効状態にする。
 * 
 * 使い方: SNSリンクを有効化するには、下の snsLinks 配列の href を実際のURLに変更するだけ。
 */
document.addEventListener('DOMContentLoaded', () => {
  const footer = document.querySelector('footer');
  if (!footer) return;

  // SNSリンクの定義（href を実際のURLに変えるとアイコンが有効化される）
  const snsLinks = [
    {
      name: 'X (Twitter)',
      href: '#',
  ...
  // SNSアイコンのHTMLを生成
  const snsHTML = snsLinks
    .map(link => {
      const isLinked = link.href !== '#';
      const linkedClass = isLinked ? ' linked' : '';
      const target = isLinked ? ' target="_blank" rel="noopener noreferrer"' : '';
      return `<a href="${link.href}" class="sns-icon${linkedClass}" aria-label="${link.name}"${target}>${link.svg}</a>`;
    })
    .join('\n        ');
  ...
  footer.innerHTML = `
    ...
    <p>&copy; 2026 Yui. All rights reserved.</p>
  `;
});
